...
const logic = (ans,cpu,w) => {
  ...
  cpu.flag_assign("PF", [...Array(v*2).keys()].map((i) => {return !!(ans & (1<<i))}).reduce((a, b) => a + b, 0))
}
...
const condjmp = (name,code,fn) => {
  instruction_define(code, (e,cpu) => {
    let disp = withsign(fetchb)
    return {
      command: name.toUpperCase(),
      args: [disp],
      run: () => {
        if (fn(cpu.flag())) {pc.assign(pc.data() + disp)}
      }
    }
  })
}
...
instruction_define("11100010", (e,cpu) => {
  ...
  return {
    ...
    run: () => {
      ...
      cx.assign(cx.data()-1)
      if (!cx.data()) {pc.assign(pc.data() + disp)}
    }
  }
})
instruction_define("11100001", (e,cpu) => {
  ...
  return {
    ...
    run: () => {
      ...
      cx.assign(cx.data()-1)
      let e = cpu.flag()
      if (!cx.data() && e.ZF) {pc.assign(pc.data() + disp)}
    }
  }
})
