...
const logic = (ans,cpu,w) => {
  ...
  cpu.flag_assign("PF", [...Array((w+1)*2).keys()].map((i) => {return !!(ans & (1<<i))}).reduce((a, b) => a + b, 0))
}
...
instruction_define("1101 00vw {mod} 100 {rm}", (e,cpu) => {
  let [v,w,mod,rm] = e
  let shift = v ? 1 : cpu.DataEle.register("CL").data()
  let obj = cpu.DataEle.r_mem(mod,rm,w)
  return {
    command: "SHL",
    args: [obj,shift],
...
instruction_define("1000 000w {mod} 100 {rm}", (e,cpu) => {
  let [w,mod,rm] = e
  let obj = cpu.DataEle.r_mem(mod,rm,w)
...
instruction_define("1000 000w {mod} 001 {rm}", (e,cpu) => {
  let [w,mod,rm] = e
  let obj = cpu.DataEle.r_mem(mod,rm,w)
  let src = cpu.DataEle.imm(cpu.fetch(w),w)
  return {
...
instruction_define("1000 000w {mod} 110 {rm}", (e,cpu) => {
  let [w,mod,rm] = e
  let obj = cpu.DataEle.r_mem(mod,rm,w)
  let src = cpu.DataEle.imm(cpu.fetch(w),w)
  return {
...
instruction_define("1110 10n1", (e,cpu) => {
  ...
  return {
    ...
    args: [disp],
    run: () => {
      let pc = cpu.DataEle.register("PC")
...
const condjmp = (name,code,fn) => {
  instruction_define(code, (e,cpu) => {
    ...
    return {
      ...
      args: [disp],
      run: () => {
        if (fn(cpu.flag())) {let pc = cpu.DataEle.register("PC"); pc.assign(pc.data() + disp)}
      }
    }
  })
}
...
instruction_define("11100010", (e,cpu) => {
  ...
  return {
    ...
    args: [disp],
    run: () => {
      let cx = cpu.register("CX")
      cx.assign(cx.data()-1)
      if (!cx.data()) {let pc = cpu.DataEle.register("PC"); pc.assign(pc.data() + disp)}
    }
  }
})
instruction_define("11100001", (e,cpu) => {
  ...
  return {
    ...
    run: () => {
      ...
      cx.assign(cx.data()-1)
      let e = cpu.flag()
      if (!cx.data() && e.ZF) {let pc = cpu.DataEle.register("PC"); pc.assign(pc.data() + disp)}
    }
  }
})
