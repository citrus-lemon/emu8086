...
class DataElement {
  constructor(str, error = defaultException) {
    ...
    // }
      ...
      let exp = _[4] + (_[3] ? ('+' + _[3]) : "")
      if (!EXP.check(exp)) {
        error.error(`expression illegal \`${exp}'`)
      }
      else {
        exp = EXP.parse(exp)
      }
      ...
      if (mod_reg == -1) {error.error("wrong register group")};
      ;(exp.isExp && (exp.sign() === '+')) && (exp = exp.filter((e) => !["BX","BP","SI","DI"].includes((''+e).toUpperCase())));
      let table
      if ([2,3,6].includes(mod_reg)) {
        table = error.me.chooseSegment('stack').labelList()
      }
      else {
        table = error.me.chooseSegment('data').labelList()
      }
      table['$'] = error.me.chooseSegment().offset
      if (exp.vaild(table)) {
        exp = exp.calc(table)
      }
      ...
      this.type = "imm"
      let table = error.me.chooseSegment('data').label
...
class SingletonCode {
  ...
  apply(me) {
    for (let i = 0; i < AssemblerCodeSet.length; i++) {
      let code = AssemblerCodeSet[i];
      if (
        (
...
function splitArgs(str, error = defaultException) {
  ...
  ans.argsmap = function(fn) {
    this.map((a,i) => (fn(a,range[i],i,ans)))
  }
  return ans
}
