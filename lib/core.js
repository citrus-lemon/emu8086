...
class CPU {
    ...
    this.DataEle.r_mem = (mod,rm,w) => {
      if (mod == 3) {
        ...
        if ((mod == 0) && (rm == 6)) {
          ...
          e.eaad = disp
          e.addr += disp
          e.sign += (mod == 0) ? "" : `+${disp.toString(16)}H`
  ...
  register_assign(reg,value) {
    if (reg.toUpperCase().match(/([A-D])([XLH])/)) {
      ...
      switch (match[2]) {
        case "X":
          return this.register_list[match[1] + "X"] = value
          break;
        case "L":
  ...
  memory_assign(mem,value, w = 0) {
    switch (w) {
      case 0:
        ...
        this.memory_block[mem] = value % 0x100
        break;
      case 1:
  ...
  flag(name) {
    if (name) {
      ...
      let ans = {}
      for (let [key,_] of Object.entries(flag_name)) {
  ...
  step() {
    let code = instruction.bind(this)()
    if (code.command) {
      code.run()
    }
    else {
